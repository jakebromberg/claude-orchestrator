...
interface DashboardColumn {
  header: string;
  width: number;
  value: (issue: Issue, status: Status, lastLogLine: string) => string;
}
...
export function renderDashboard(options: RenderOptions): string {
  ...
  for (const issue of config.issues) {
    ...
    else if (status === "running" || status === "interrupted") color = YELLOW;
    else if (status === "skipped") color = DIM;

    const cells = COLUMNS.map((c) =>
      c.value(issue, status, logLine).padEnd(c.width).slice(0, c.width),
    );
  ...
  lines.push(
    `  ${GREEN}Succeeded: ${succeeded}${NC}  ${RED}Failed: ${failed}${NC}  ${YELLOW}Running: ${running}${NC}  Pending: ${pending}  ${DIM}Skipped: ${skipped}${NC}  Total: ${config.issues.length}`,
  );
  lines.push("");
...
function defaultReadFileTail(filePath: string, bytes: number): string {
...
export function readLastLogLine(
  logPath: string,
  readFileTail: ReadFileTail = defaultReadFileTail,
): string {
  try {
