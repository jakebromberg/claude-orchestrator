  ...
  return (issues, getStatus) => {
    ...
    for (const issue of issues) {
      ...
      const cells = columns.map((c) =>
        c.value(issue, status).padEnd(c.width).slice(0, c.width),
      );
    ...
    let totalsLine =
      `  ${GREEN}Succeeded: ${succeeded}${NC}  ${RED}Failed: ${failed}${NC}  ${YELLOW}Running: ${running}${NC}  Pending: ${pending}  ${DIM}Skipped: ${skipped}${NC}  Total: ${issues.length}`;
