...
export async function decompose(
  ...
  if (input.issueNumber && input.repo) {
    const issueBody = deps.runCommand(
      `gh issue view ${input.issueNumber} --repo ${input.repo} --json body -q .body`,
    );
  ...
  const rawOutput = deps.runCommand(
    `claude -p ${escapeShellArg(fullPrompt)} --output-format json`,
  );
  ...
  if (!parsed || typeof parsed !== "object" || !Array.isArray((parsed as Record<string, unknown>).issues)) {
    throw new Error("Claude output missing required 'issues' array");
  }

  const rawIssues = (parsed as { issues: DecomposedIssue[] }).issues;
...
function generateYamlFragment(issues: DecomposedIssue[]): string {
