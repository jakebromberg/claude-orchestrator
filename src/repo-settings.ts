 ...
 */
export function unknownRepoKeys(yaml: YamlConfig): string[] {
  if (!yaml.repos) return [];
  const known = new Set<string>();
  if (yaml.defaultRepo) known.add(yaml.defaultRepo);
  for (const issue of yaml.issues) {
    if (issue.repo) known.add(issue.repo);
  }
