...
export function parseClaimArgs(argv: string[]): ClaimArgs {
  ...
  while (i < argv.length) {
    ...
    switch (arg) {
      ...
      case "--issue": {
        const v = argv[i + 1];
        if (!v) throw new Error("--issue requires a number");
        const n = parseInt(v, 10);
        if (!Number.isFinite(n)) throw new Error("--issue must be a number");
        issue = n;
        i += 2;
        break;
      }
      case "--domain": {
        const v = argv[i + 1];
        if (!v) throw new Error("--domain requires a name");
        domain = v;
        i += 2;
        break;
      }
...
export function runClaim(opts: RunClaimOptions): string {
