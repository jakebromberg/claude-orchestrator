...
export type PlanPreviewConfig = Pick<
  OrchestratorConfig,
  "name" | "issues" | "defaultModel" | "defaultEffort"
> & { defaultRepo?: string };
...
function issueDetail(issue: Issue, config: PlanPreviewConfig): string {
  if (isModeNode(issue)) {
    const kind = `[${issue.mode}]`;
    if (isManualGate(issue)) return `${kind} manual gate`;
...
function renderIssueLine(
  ...
  if (issue.deps.length > 0) parts.push(`deps: ${issue.deps.join(", ")}`);
  if (gateReason) parts.push(`⚠ HITL: ${gateReason}`);
  return parts.join("  ");
}
 ...
 */
export function renderPlanPreview(config: PlanPreviewConfig): string {
  const { issues } = config;
  const byRef = new Map<string, Issue>(issues.map((i) => [i.ref, i]));
  const lookup = (ref: string): Issue | undefined => byRef.get(ref);

  // Which issues stop for a human, and why.
  const gateReason = new Map<string, string>();
  for (const issue of issues) {
    const reason = cutoverReason(issue, lookup);
    if (reason) gateReason.set(issue.ref, reason);
  }
  ...
  if (gateCount > 0) {
    lines.push(
      `HITL cutover gates (${gateCount}) — the run holds here until \`confirmCutover\` approves:`,
    );
