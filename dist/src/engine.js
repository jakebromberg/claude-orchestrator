...
export class Orchestrator {
     ...
     */
    reportHeldGates() {
        const held = this.config.issues.filter((i) => this.deps.statusStore.get(i.ref) === "pending");
        if (held.length === 0)
            return;
        const skipped = this.config.issues.filter((i) => this.deps.statusStore.get(i.ref) === "skipped").length;
        this.deps.logger.warn(`Run finished with ${held.length} issue(s) held at a cutover gate` +
    ...
    async prepareIssues(issues) {
        ...
        for (const issue of issues) {
            ...
            if (gateReason && !(await this.confirmCutover(issue, gateReason))) {
                this.deps.logger.warn(`${issue.ref} held at cutover gate (${gateReason}); awaiting ` +
    ...
    async runModeNodes(modeNodes) {
        for (const issue of modeNodes) {
            ...
            this.deps.logger.step(`Running ${issue.mode} node ${issue.ref}: ${issue.command}`);
            ...
            try {
                ...
                await this.setStatus(issue, "failed");
                this.deps.logger.error(`${issue.ref} (${issue.mode}) failed (exit ${exitCode}): ${detail}`);
            }
        }
    }
    /**
    ...
    producedNoCommits(issue) {
        ...
        try {
            ...
            raw = this.deps.runCommand(`git -C "${worktreePath}" rev-list --count origin/${shellQuote(baseBranch)}..HEAD`);
        }
        catch (err) {
        ...
        if (!/^\d+$/.test(trimmed)) {
            this.deps.logger.warn(`Issue #${issue.number}: unexpected commit count ${JSON.stringify(trimmed)}; ` +
                `leaving the session's own result in place`);
            return false;
        }
        return trimmed === "0";
     ...
     */
    async recordCleanExit(issue, note, logFile) {
        if (this.producedNoCommits(issue)) {
            await this.setStatus(issue, "failed");
            this.deps.logger.error(`Issue #${issue.number} exited cleanly but left no commits on ` +
    ...
    async handleCheckResultWithRetry(issue, checkResult, originalPrompt, worktreePath, logFile, stderrFile) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (checkResult.passed) {
                ...
                this.deps.logger.info(`Issue #${issue.number} passed checks after retry ${attempt}`);
                return true;
            }
        }
