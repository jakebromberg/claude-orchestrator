 ...
 */
export function usesApiKeyBilling(env = process.env) {
 ...
 */
export function claudeExecOptions(extra = {}) {
    return {
        encoding: "utf-8",
 ...
 */
export function authBanner(env = process.env) {
    if (usesApiKeyBilling(env)) {
        return {
            level: "warn",
            message: `Auth: Anthropic API key (${USE_API_KEY_ENV_VAR} is set) — sessions bill API credits`,
        };
    }
    ...
    if (ignored.length > 0) {
        return {
            level: "info",
            message: `Auth: Claude Code login — ${ignored.join(" and ")} ignored ` +
 ...
 */
export function claudeSessionEnv(env = process.env, options = {}) {
