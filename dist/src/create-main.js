...
export async function createMain(options) {
    ...
    if (usesApiKeyBilling()) {
        deps.logger.warn(`Auth: ANTHROPIC_API_KEY (${USE_API_KEY_ENV_VAR} is set) — sessions bill Anthropic API credits`);
    }
    else {
    ...
    function collectAndWriteRunRecord() {
    ...
    const finishedAt = new Date();
    const report = generateReport(config.name, config.issues, (n) => deps.statusStore.get(n), (n) => deps.metadataStore.get(n), startedAt, finishedAt);
