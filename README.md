...
Confirmation comes from an optional `confirmCutover(issue, reason)` hook (return `true` to release, `false` to hold). **When the hook is absent, gated issues hold** — they're left `pending`, their dependents skip, and a later full re-run (`run-all`, once the upstream is live or with the hook wired) re-evaluates the gate and picks them up. Note that `--retry-failed` does **not** resume a held gate — a held gate is `pending`, not `failed`, so only a full `run-all` revisits it; the end-of-run summary lists what was held. This is the conservative default: a cross-repo run must wire `confirmCutover`, or model explicit command mode-nodes, to progress unattended. So `publish → BS consumes → deploy → canary` flows automatically when the publish/deploy/canary steps are command mode-nodes, and only a raw cross-repo hand-off pauses for a human.
...
`succeeded` is a claim that the run's work landed, not that its process exited cleanly — an agent that ends its turn mid-task exits 0. Before recording it, the engine counts commits on the issue's branch (`git rev-list --count origin/<base>..HEAD`, with `<base>` from the optional `getBaseBranch` hook, falling back to `"main"`); a clean exit over a branch holding zero commits is recorded as `failed`. Provide that hook for any repo whose default branch is not `main` — `deriveWorktreeHooks` does — because the fallback makes the count run against a ref that does not exist, and the check then degrades to a warning instead of a guard. When the count can't be determined at all — worktree already removed, no `origin/<base>` ref, git unavailable — the engine warns and leaves the session's own result in place. Only positive evidence of zero commits downgrades a run.

The PR a session opened is recovered by scraping its log, and is verified before it is recorded. Candidates are filtered to the issue's own `owner/repo` — which requires a repo to compare against, so the filter applies only when the issue carries a `repo` or the run sets `defaultRepo`; without either, every PR URL in the log stays a candidate and the identity check below is the only thing standing between a quoted URL and the store. Each candidate is then checked newest-first with `gh pr view <n> --repo <owner/repo> --json state,headRefName`, which must confirm the PR exists, is open, and has this run's branch as its head; the first that passes is recorded. Verifying more than one matters because the log is appended across retries and a same-repo PR URL can be printed *after* the real one (upstream handoff notes quoted into `UPSTREAM_CONTEXT` do exactly that), so a single rejection is not proof the session opened nothing. If none verify, the scrape is dropped with a warning and existing metadata is left untouched. The one exception to leaving metadata alone is a `prUrl` carrying no `prVerifiedAt`: it was written before provenance was tracked and so was never proven to be this issue's, and it is re-checked and discarded if it fails. This matters beyond reporting — `--merge` hands `metadata.prUrl` straight to `gh pr merge`, so a PR URL merely quoted somewhere in a multi-megabyte transcript must never reach it.
