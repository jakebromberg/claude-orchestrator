...
describe("InMemoryStatusStore", () => {
  ...
  it("keeps colliding numbers in different repos distinct", () => {
    store.set("WXYC/lml#924", "running");
    store.set("WXYC/backend#924", "failed");
    expect(store.get("WXYC/lml#924")).toBe("running");
    expect(store.get("WXYC/backend#924")).toBe("failed");
  });
...
describe("FileStatusStore", () => {
  ...
  it("writes a bare-number ref to the legacy issue-N.status filename (back-compat)", () => {
    store.set("5", "running");
  ...
  it("encodes / and # in a cross-repo ref filename and round-trips", () => {
    store.set("WXYC/lml#924", "succeeded");
    const filePath = path.join(tmpDir, "status", "issue-WXYC%2Flml%23924.status");
    expect(fs.existsSync(filePath)).toBe(true);
    expect(store.get("WXYC/lml#924")).toBe("succeeded");
  });
...
describe("FileMetadataStore", () => {
  ...
  it("writes a bare-number ref to the legacy issue-N.json filename (back-compat)", () => {
  ...
  it("encodes a cross-repo ref filename and round-trips", () => {
    store.set("WXYC/lml#924", { exitCode: 0 });
    const filePath = path.join(tmpDir, "metadata", "issue-WXYC%2Flml%23924.json");
    expect(fs.existsSync(filePath)).toBe(true);
    expect(store.get("WXYC/lml#924")).toEqual({ exitCode: 0 });
  });
